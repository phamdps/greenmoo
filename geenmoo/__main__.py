...
def main(input=None):
    ...
    def load_set(file):
        ...
        if file is None:
            return greenmoo.load(sys.stdin)

        try:
            return greenmoo.load_json(file)
        except json.decoder.JSONDecodeError:
            return greenmoo.load_objectives(file)

    def save_set(result, file, indent=4):
        """Output result to stdout or file."""
        if file is None:
            greenmoo.dump(result, sys.stdout, indent=indent)
            sys.stdout.write(os.linesep)
        else:
            greenmoo.save_json(file, result, indent=indent)
    ...
    parser.add_argument("-v", "--version", action="version", version=greenmoo.PlatypusConfig.version)
    ...
    if args.command == "hypervolume":
        ...
        hyp = greenmoo.Hypervolume(reference_set=ref_set)
        print(hyp.calculate(input_set))
    elif args.command == "gd":
        ref_set = load_set(args.reference_set)
        input_set = load_set(args.filename)
        gd = greenmoo.GenerationalDistance(reference_set=ref_set)
        print(gd.calculate(input_set))
    elif args.command == "igd":
        ref_set = load_set(args.reference_set)
        input_set = load_set(args.filename)
        igd = greenmoo.InvertedGenerationalDistance(reference_set=ref_set)
        print(igd.calculate(input_set))
    elif args.command == "epsilon":
        ref_set = load_set(args.reference_set)
        input_set = load_set(args.filename)
        eps = greenmoo.EpsilonIndicator(reference_set=ref_set)
        print(eps.calculate(input_set))
    elif args.command == "spacing":
        input_set = load_set(args.filename)
        spacing = greenmoo.Spacing()
        ...
        if args.problem not in dir(problem_module):
            raise greenmoo.PlatypusError(f"'{args.problem}' not found in module '{problem_module.__name__}'")
        if args.algorithm not in dir(algorithm_module):
            raise greenmoo.PlatypusError(f"'{args.algorithm}' not found in module '{algorithm_module.__name__}'")

        problem_class = getattr(problem_module, args.problem)
        algorithm_class = getattr(algorithm_module, args.algorithm)

        if not issubclass(problem_class, greenmoo.Problem):
            raise greenmoo.PlatypusError(f"'{args.problem}' is not a valid Problem")
        if not issubclass(algorithm_class, greenmoo.Algorithm):
            raise greenmoo.PlatypusError(f"'{args.algorithm}' is not a valid Algorithm")
        ...
        if args.unique:
            input_set = greenmoo.unique(input_set)
        if args.feasible:
            input_set = greenmoo.feasible(input_set)
        if args.nondominated or args.epsilons:
            archive = greenmoo.EpsilonBoxArchive(args.epsilons) if args.epsilons else greenmoo.Archive()
            archive += input_set
            input_set = archive
        ...
        if args.reference_set:
            ...
            ref_set = load_set(args.reference_set)
            minimum, maximum = greenmoo.normalize(ref_set)

        norm_min, norm_max = greenmoo.normalize(input_set, minimum, maximum)
        ...
        if nobjs == 2:
            ...
            raise greenmoo.PlatypusError("plot requires a set with 2 or 3 objectives")
